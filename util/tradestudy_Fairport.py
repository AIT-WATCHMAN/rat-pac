...
dAIR =   zROCK - (zPMT + dTANK)


print('Air volume:',dAIR)
## Values to change for PMT arrangement. (PMTINFO)
#photocoverage = 0.10
#photocoverage = 0.1505
#photocoverage = 0.205
photocoverage = float(argv[2])
pmtRad        = 126.5
...
type: "box",
size: [{xROCK+dTANK+dAIR+dROCK+dAIR+xPMT}, {yROCK+dTANK+dAIR+dROCK+dAIR+xPMT}, {zROCK+dTANK+dAIR+dROCK+dAIR+xPMT}], // mm, half-length
...
size: [{xROCK+dROCK}, {yROCK+dROCK}, {zROCK+dROCK}],
...
size: [{xROCK+dCONC}, {yROCK+dCONC}, {zROCK+dCONC}], 
...
size: [{xROCK-dIBEAM+tIBEAM}, {yROCK-dIBEAM+tIBEAM}, {zROCK-dIBEAM+tIBEAM}], 
...
type: "box",
//size: [5000.0, 26000.0, 5000.0], // mm, half-length
size: [{xROCK-dIBEAM}, {yROCK-dIBEAM}, {zROCK-dIBEAM}], 
position: [0.0, 0.0, 0.0],
material: "air",
color: [0.85, 0.72, 1.0, 0.5],
invisible: 0,
...
position: [0.0, 0.0, {oTANK+dIBEAM+tTANK-dAIR}],
material: "stainless_steel",
...
try:
    os.mkdir(f"../data/Watchman_fairport_{int((rPMT+dTANK)*2.0/1000)}m_{int((zPMT+dTANK)*2.0/1000)}m_{int(photocoverage*100)}pct_{int(rPMT)}mm")
    print('Created', f"../data/Watchman_fairport_{int((rPMT+dTANK)*2.0/1000)}m_{int((zPMT+dTANK)*2.0/1000)}m_{int(photocoverage*100)}pct_{int(rPMT)}mm")
except OSError as error:  
    print(error)   

geofile = open(f"../data/Watchman_fairport_{int((rPMT+dTANK)*2.0/1000)}m_{int((zPMT+dTANK)*2.0/1000)}m_{int(photocoverage*100)}pct_{int(rPMT)}mm/Watchman_fairport_{int((rPMT+dTANK)*2.0/1000)}m_{int((zPMT+dTANK)*2.0/1000)}m_{int(photocoverage*100)}pct_{int(rPMT)}mm.geo","w+")
geofile.writelines(_geoFile)
geofile.close

pmtfile = open(f"../data/Watchman_fairport_{int((rPMT+dTANK)*2.0/1000)}m_{int((zPMT+dTANK)*2.0/1000)}m_{int(photocoverage*100)}pct_{int(rPMT)}mm/PMTINFO.ratdb","w+")
