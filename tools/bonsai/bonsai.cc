...
int main(int argc, char **argv) {
  ...
  printf(
      "\n\nWelcome to BONSAI. The function can take no less than two input and "
      "up\n");
  printf(
      "to 6 inputs: infile,outfile,darkNoise,timeOffset,minTime,maxtime\n\n");
  ...
  switch (argc) {
    case 3:
      printf(
          "Only input file and output file are provided. All other values set "
          "to default.\n");
      break;
    case 4:
      printf(
          "Only input file and output file and dark noise rate are provided. "
          "All other values set to default.\n");
      darkNoise = float(strtol(argv[3], NULL, 10));
      break;
    case 5:
      // printf("Only input file and output file are provide. All other values
      // set to default.\n");
      darkNoise = float(strtol(argv[3], NULL, 10));
      offsetT = float(strtol(argv[4], NULL, 10));
      break;
    case 6:
      // printf("Only input file and output file are provided. All other values
      // set to default.\n");
      darkNoise = float(strtol(argv[3], NULL, 10));
      offsetT = float(strtol(argv[4], NULL, 10));
  ...
  data->Branch("id_plus_dr_hit", &tot_nhit,
               "id_plus_dr_hit/I");  // Inner detector plus dark rate hits
  data->Branch("veto_hit", &veto_hit, "veto_hit/I");  // veto detector
  data->Branch("veto_plus_dr_hit", &totVHIT,
               "veto_plus_dr_hit/I");  // veto detector plus dark rate hits
  ...
  data->Branch("y", &y, "y/D");
  data->Branch("z", &z, "z/D");
  data->Branch("t", &tim, "t/D");
  data->Branch("u", &u, "u/D");
  data->Branch("v", &v, "v/D");
  data->Branch("w", &w, "w/D");
  data->Branch("particleCountMC", &particleCountMC, "particleCountMC/I");
  data->Branch("mc_energy", &mc_energy, "mc_energy/D");
  data->Branch("mcx", &mc_x, "mcx/D");
  data->Branch("mcy", &mc_y, "mcy/D");
  data->Branch("mcz", &mc_z, "mcz/D");
  data->Branch("mct", &mc_tim, "mct/D");
  data->Branch("mcu", &mc_u, "mcu/D");
  data->Branch("mcv", &mc_v, "mcv/D");
  data->Branch("mcw", &mc_w, "mcw/D");
  // data->Branch("code",&code,"code/I");
  data->Branch("closestPMT", &closestPMT,
               "closestPMT/D");  // Proximity to PMT wall
  ...
  // generate BONSAI geometry object
  {
    ...
    if (count != tot_inner)
      printf("There is a descreptancy in inner PMTS %d vs %d", count,
             tot_inner);

    // create BONSAI objects from the PMT position array
    bsgeom = new pmt_geometry(inpmt, xyz);
    bslike =
        new likelihood(bsgeom->cylinder_radius(), bsgeom->cylinder_height());
  ...
  for (event = 0; event < n_events; event++) {
    ...
    for (sub_event = 0; sub_event < ds->GetEVCount(); sub_event++) {
      ...
      bsgdn = new goodness(bslike->sets(), bslike->chargebins(), bsgeom, nhit,
                           cables, times, charges);
      ...
      *bonsai_vtxfit = bsfit->xfit();
      ...
      int n9win =
          nwin(pmtinfo, 9, bonsai_vtxfit, nhit, cables, times, cables_win);
      float apmt[3 * n9win];
      ...
      lfariadne_(bonsai_vtxfit, &n9win, apmt, adir, &agoodn, &aqual, &nsct,
                 &cosscat);
      dir_goodness = agoodn;
      u = adir[0];
      v = adir[1];
...
int nwin(RAT::DS::PMTInfo *pmtinfo, float twin, float *v, int nfit, int *cfit,
  ...
  for (hit = 0; hit < nfit; hit++) {
    ...
    dz = pos.Z() * 0.1 - v[2];
    tsort[hit] = ttof[hit] =
        tfit[hit] - sqrt(dx * dx + dy * dy + dz * dz) * CM_TO_NS;
  }
