...
namespace RAT {
    ...
    void LetterboxFactory::DefineDetector(DBLinkPtr /*detector*/) {
        DB *db = DB::Get();
        DBLinkPtr params = db->GetLink("LETTERBOX_PARAMS");
        ...
        const std::string eh_template = "Watchman_Letterbox/ExperimentalHall.geo";
        if (db->Load(eh_template) == 0) {
            Log::Die("LetterboxFactory: could not load template Watchman_Letterbox/ExperimentalHall.geo");
        }
        const std::string dm_template = "Watchman_Letterbox/DetectorModule.geo";
        if (db->Load(dm_template) == 0) {
            Log::Die("LetterboxFactory: could not load template Watchman_Letterbox/DetectorModule.geo");
        }
        /*
