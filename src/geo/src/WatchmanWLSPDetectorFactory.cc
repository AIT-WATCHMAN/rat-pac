...
namespace RAT {
    ...
    void WatchmanWLSPDetectorFactory::DefineDetector(DBLinkPtr /*detector*/) {
        DB *db = DB::Get();
        ...
        DBLinkPtr wlsp                = db->GetLink("GEO","WLSP");
        DBLinkPtr wlspcover           = db->GetLink("GEO","WLSP_reflector_back");
        double wlspz                  = (wlsp->GetDArray("z")[1])+(wlspcover->GetD("size_z"));
        ...
        if (photocathode_coverage == 0.00) {
          for (size_t i = 0; i < pmtinfox.size(); i++) {
            x[i] = pmtinfox[i];
            y[i] = pmtinfoy[i];
            z[i] = pmtinfoz[i];
        ...
        if (photocathode_coverage == 0.00) info<<"Using dfault PMTINFO information...\n";
        ...
        if (photocathode_coverage != 0.00) {
          ...
          db->SetDArray("GEO","WLSP","r_max",{(pmt_space/2.0)-8.0,(pmt_space/2.0)-8.0});
          db->SetDArray("GEO","WLSP","r_min",{rho_edge[1]+5.0,rho_edge[1]-15.0});
          db->SetD("GEO","WLSP_reflector","r_min",(pmt_space/2.0)-8.0);
          db->SetD("GEO","WLSP_reflector","r_max",(pmt_space/2.0)-7.0);
          db->SetD("GEO","WLSP_reflector_back","r_max",(pmt_space/2.0)-7.0);
          db->SetD("GEO","WLSP_reflector_back","r_min",rho_edge[1]+5.0);

        }
        else {
        
          db->SetDArray("GEO","WLSP","r_max",{240.0,240.0});
          db->SetDArray("GEO","WLSP","r_min",{rho_edge[1]+5.0,rho_edge[1]-15.0});
          db->SetD("GEO","WLSP_reflector","r_min",240.0);
          db->SetD("GEO","WLSP_reflector","r_max",242.0);
          db->SetD("GEO","WLSP_reflector_back","r_max",242.0);
          db->SetD("GEO","WLSP_reflector_back","r_min",rho_edge[1]+5.0);
        }
