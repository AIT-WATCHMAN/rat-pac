...
int bsnwin3_(float *vertex, float *tmin, float *tmax, float *ttof, float *d,
             int *cab);
...
void bsloadinit_(float *res) {
  vf_ = new vertexfit(*res);
  // gri=new gringo(vf_);
}

void bsinit_(int *np, float *ps, float *res) {
  vf_ = new vertexfit(*np, ps, *res);
  // gri=new gringo(vf_);
}
...
void bsloadhits_(int *nhit, int *cab, int *badch, float *t, float *q) {
  vf_->loadhits(*nhit, cab, badch, t, q);
}

float bstimedirfit_(int *fast, float *vertex, float *direct, float *dt) {
  if (*fast > 1)
    return (vf_->fittime_lcor((*fast) - 2, vertex, direct, *dt));
  else
    return (vf_->fittime(*fast, vertex, direct, *dt));
}

void bssetcang_(float *cang0, float *pang, float *mang) {
  vf_->set_cang(*cang0, *pang, *mang);
}
...
float bsbestvertex_(float *vertex) {
  float gdn[vf_->sets()], ll;

  vf_->bestvertex(vertex);
  return (vf_->goodness(ll, vertex, gdn));
}
...
float bsfitvertex_(float *vertex, float *rad, int *maxiter) {
  ...
  vf_->tree::search(vertex, *rad, *maxiter);
  vf_->bestvertex(vertex);
  return (vf_->goodness(ll, vertex, gdn));
}

void bsaddvertex_(float *vertex) { vf_->addvertex(vertex, 50.); }

float bsgood_(float *vertex, float *bgrate, float *guncor) {
  return (vf_->tgood(vertex, *bgrate, *guncor));
}

int bsnwin1_(float *vertex, float *tmin, float *tmax) {
  return (vf_->nwind(vertex, *tmin, *tmax));
}

int bsnwin2_(float *vertex, float *tmin, float *tmax, float *ttof, int *cab) {
  return (vf_->nwind(vertex, *tmin, *tmax, ttof, cab));
}

int bsnwin3_(float *vertex, float *tmin, float *tmax, float *ttof, float *d,
             int *cab) {
  return (vf_->nwind(vertex, *tmin, *tmax, ttof, d, cab));
}

float bsbestwall_(float *dwall, float *rmin, float *wallv) {
  return (vf_->bestwall(*dwall, *rmin, wallv));
}
...
void qsort(float *cabs, short int *list, short int n, short int *first) {
  ...
  for (firstp = secondp = 0;
       (secondp < m) && ((firstp < half) || (nfirst != 0)); firstp++) {
    if (cabs[firstel] <
        cabs[second[secondp]]) {         // take element of the first sublist
...
void it_clus_and_bonsai_fit(float *clusvert, float *bonsaivert,
                            ...
                            float *maxgood, float *maxlike, int *nsel,
                            int *nclusfit, int *nbonsaifit, float cluswallcut,
                            comtype2 *itevent) {
  *clusvert = clusvert[1] = clusvert[2] = 0;
  *maxgood = clusvert[3] = -1;
  goodness good(tf->sets(), tf->chargebins(), geom, itevent);
  *nsel = good.nselected();
...
void it_clus_bonsai_fit(float *clusvert, float *bonsaivert, float *clusresult,
                        float *bonsairesult, float *maxgood, float *maxlike,
...
void it_vfwrite(FILE *itvw, unsigned int event_number, int trigger_type,
                ...
                unsigned int fit_tubes, unsigned int cfit,
                unsigned int cvertex_tubes, float *cvtx, float *cdir,
                float cgof, unsigned int bfit, unsigned int bvertex_tubes,
                float *bvtx, float *bdir, float bgof, float bll, float bll0) {
  fprintf(itvw, "%8d %2d %1d %8d %5d %5d %5d %3d ", event_number, trigger_type,
          keep, flag, clock48_high, clock48_middle, clock48_low, fit_tubes);
  fprintf(itvw, "%4d %3d %8.2f %8.2f %8.2f %6.1f %6.4f %7.4f %6.4f %6.4f ",
          cfit, cvertex_tubes, cvtx[0], cvtx[1], cvtx[2], cvtx[3], cdir[0],
          cdir[1], cdir[2], cgof);
  fprintf(itvw,
          "%4d %3d %8.2f %8.2f %8.2f %6.1f %6.4f %7.4f %7.4f %6.4f %6.4f %6.4f "
          "%8.3f %8.3f\n",
          bfit, bvertex_tubes, bvtx[0], bvtx[1], bvtx[2], bvtx[3], bdir[0],
          bdir[1], bdir[2], bdir[3], bdir[4], bgof, bll, bll0);
}
...
short int it_vfread(FILE *itvr, unsigned int *event_number, int *trigger_type,
                    ...
                    unsigned int *fit_tubes, unsigned int *cfit,
                    unsigned int *cvertex_tubes, float *cvtx, float *cdir,
                    float *cgof, unsigned int *bfit,
                    unsigned int *bvertex_tubes, float *bvtx, float *bdir,
                    float *bgof, float *bll, float *bll0) {
  char line[256];

  fgets(line, 255, itvr);
  if (feof(itvr)) return (-1);
  sscanf(line,
         "%d %d %d %d %hd %hd %hd %d %d %d %f %f %f %f %f %f %f %f %d %d %f %f "
         "%f %f %f %f %f %f %f %f %f %f",
         event_number, trigger_type, keep, flag, clock48_high, clock48_middle,
         clock48_low, fit_tubes, cfit, cvertex_tubes, cvtx, cvtx + 1, cvtx + 2,
         cvtx + 3, cdir, cdir + 1, cdir + 2, cgof, bfit, bvertex_tubes, bvtx,
         bvtx + 1, bvtx + 2, bvtx + 3, bdir, bdir + 1, bdir + 2, bdir + 3,
         bdir + 4, bgof, bll, bll0);
  return (0);
}
...
void storetest_(int *nhit, int *cab, float *t, float *q, float *tsig,
                float *dwallmin, int *nsel, float *goodn) {
  goodness good(tf->sets(), tf->chargebins(), geom, *nhit, cab, t, q);
  if ((*nsel = good.nselected()) < MINHIT) {
    *goodn = -1;
    return;
  }
  fourhitgrid gr(geom->cylinder_radius(), geom->cylinder_height(), &good, *tsig,
                 *dwallmin, *goodn);
}

int clusfit_(float *vert, float *result, float *maxgood, int *nsel, int *nhit,
             int *cab, float *t, float *q) {
  goodness good(tf->sets(), tf->chargebins(), geom, *nhit, cab, t, q);
  *nsel = good.nselected();
  if (good.nselected() < MINHIT) return (0);
  fourhitgrid gr(geom->cylinder_radius(), geom->cylinder_height(), &good);
...
int bonsaifit_(float *vert, float *result, float *maxlike, int *nsel, int *nhit,
               int *cab, float *t, float *q) {
...
void clusbonsaifit_(float *clusvert, float *bonsaivert, float *clusresult,
                    float *bonsairesult, float *maxgood, float *maxlike,
                    int *nsel, int *nclusfit, int *nbonsaifit, int *nhit,
                    int *cab, float *t, float *q) {
  goodness good(tf->sets(), tf->chargebins(), geom, *nhit, cab, t, q);
  *nsel = good.nselected();
  *nclusfit = *nbonsaifit = 0;
...
void clusandbonsaifit_(float *clusvert, float *bonsaivert, float *clusresult,
                       float *bonsairesult, float *maxgood, float *maxlike,
                       int *nsel, int *nclusfit, int *nbonsaifit,
                       float *cluswallcut, int *nhit, int *cab, float *t,
                       float *q) {
  goodness good(tf->sets(), tf->chargebins(), geom, *nhit, cab, t, q);
  *nsel = good.nselected();
  *nclusfit = *nbonsaifit = 0;
  ...
  *nbonsaifit = -1;
  ...
  if ((*nclusfit = good.nfit()) != 0) {
    ...
    *clusresult = good.get_theta();
    ...
    clusresult[3] = good.get_gdn0();
    rwall =
        good.returnrvol() - sqrt(cf.xfit() * cf.xfit() + cf.yfit() * cf.yfit());
...
void goodness_(float *guncor, float *vertex, int *nhit, int *cab, float *t,
               float *q) {
  // hitsel event_hits(tf->sets(),tf->chargebins(),geom,*nhit,cab,t,q);
  hitsel event_hits(0, 0, 0, 0, 0, tf->sets(), tf->chargebins(), geom, *nhit,
                    cab, t, q);
...
int nwin1_(float *vertex, float *tmin, float *tmax, int *nhit, int *cab,
           float *t, float *q) {
  goodness good(tf->sets(), tf->chargebins(), geom, *nhit, cab, t, q);
...
int nwin3_(float *vertex, float *tmin, float *tmax, float *ttof, float *d,
           int *cab) {
...
void bsfilereadopen_(char *name, int length) {
  ...
  for (i = 0; i < length; i++) cname[i] = name[i];
  cname[i] = 0;
...
void bsfileclose_(void) { delete bsfile; }
