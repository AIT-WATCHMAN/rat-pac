...
// * array or variable                                         *
// *************************************************************
char fit_param::parse(char *line, unsigned char length, char *key1, char *key2,
                      char *key3, char type, void *var) {
...
inline void fit_param::parse(char *line) {
  ...
  if (parse(line, length, "PMT", "oincidence", "ime", FLOAT_TYPE, &tcoin))
    return;
  if (parse(line, length, "air", "istance", "raction", FLOAT_TYPE, &dlim))
    return;
  if (parse(line, length, "air", "ime", "raction", FLOAT_TYPE, &tlim)) return;
  if (parse(line, length, "aximum", "its", "ombin", SHORT_INT_TYPE, &nselall))
    return;
  if (parse(line, length, "nitial", "rid", "onstant", FLOAT_TYPE, &gcon0))
    return;

  if (parse(line, length, "lusfit", "rid", "onstant", ARRAY_FLOAT_TYPE, &cgrid))
    return;
  if (parse(line, length, "all", "istance", "ertices", FLOAT_TYPE, &dw4hit))
    return;
  if (parse(line, length, "all", "lusfit", "ertex", FLOAT_TYPE, &dwfit)) return;
  if (parse(line, length, "nitial", "Clusfit", "pening", FLOAT_TYPE,
            &clusgtheta0))
    return;
  if (parse(line, length, "nitial", "Clusfit", "ositive", FLOAT_TYPE,
            &clusgthetaplus))
    return;
  if (parse(line, length, "nitial", "Clusfit", "egative", FLOAT_TYPE,
            &clusgthetaminus))
    return;
  if (parse(line, length, "nitial", "Clusfit", "eight", FLOAT_TYPE,
            &clusgdirweight))
    return;
  if ((np == -1) &&
      (parse(line, length, "umber", "lusfit", "asses", SHORT_INT_TYPE, &np))) {
    if (np < 1) {
      np = -1;
      return;
    }
    ...
    for (ar = 0; ar < np; ar++) {
      ...
      clustheta0[ar] = clusthetaplus[ar] = clusthetaminus[ar] =
          clusdirweight[ar] = -1;
    }
    return;
  }
  if (parse(line, length, "inal", "oodness", "ifference", FLOAT_TYPE,
            &lastdiff))
    return;
  if ((np != -1) &&
      (parse(line, length, "all", "nvoke", "iner", ARRAY_FLOAT_TYPE, cdwall)))
    return;
  if ((np != -1) && (parse(line, length, "oodness", "ifference", "kim",
                           ARRAY_FLOAT_TYPE, gdiff)))
    return;
  if (parse(line, length, "inal", "oodness", "raction", FLOAT_TYPE, &lastfrac))
    return;
  if ((np != -1) && (parse(line, length, "oodness", "kim", "raction",
                           ARRAY_FLOAT_TYPE, gfrac)))
    return;
  if (parse(line, length, "rid", "ime", "indow", FLOAT_TYPE, &tim0)) return;
  if ((np != -1) &&
      (parse(line, length, "oodness", "ime", "indow", ARRAY_FLOAT_TYPE, time)))
    return;
  if ((np != -1) && (parse(line, length, "lusfit", "inimum", "adius",
                           ARRAY_FLOAT_TYPE, rmin)))
    return;
  if ((np != -1) &&
      (parse(line, length, "lusfit", "top", "adius", ARRAY_FLOAT_TYPE, rstop)))
    return;
  if (parse(line, length, "Clusfit", "pening", "ngle", ARRAY_FLOAT_TYPE,
            clustheta0))
    return;
  if (parse(line, length, "Clusfit", "ositive", "ngle", ARRAY_FLOAT_TYPE,
            clusthetaplus))
    return;
  if (parse(line, length, "Clusfit", "egative", "ngle", ARRAY_FLOAT_TYPE,
            clusthetaminus))
    return;
  if (parse(line, length, "Clusfit", "ngle", "eight", ARRAY_FLOAT_TYPE,
            clusdirweight))
    return;
  if (parse(line, length, "BONSAI", "rid", "onstant", FLOAT_TYPE, &bgrid))
    return;
  if (parse(line, length, "all", "BONSAI", "ertex", FLOAT_TYPE, &bdwfit))
    return;
  if (parse(line, length, "all", "nvoke", "ine", FLOAT_TYPE, &bdwall)) return;
  if (parse(line, length, "nitial", "pening", "ngle", FLOAT_TYPE, &bc0)) return;
  if (parse(line, length, "nitial", "ngle", "ositive", FLOAT_TYPE, &bpangle))
    return;
  if (parse(line, length, "nitial", "ngle", "egative", FLOAT_TYPE, &bmangle))
    return;
  if (parse(line, length, "inal", "ikelihood", "ifference", FLOAT_TYPE,
            &blastdiff))
    return;
  if (parse(line, length, "inal", "ikelihood", "raction", FLOAT_TYPE,
            &blastfrac))
    return;
  if (parse(line, length, "oarse", "pening", "ngle", FLOAT_TYPE, &oc0)) return;
  if (parse(line, length, "oarse", "ngle", "ositive", FLOAT_TYPE, &opangle))
    return;
  if (parse(line, length, "oarse", "ngle", "egative", FLOAT_TYPE, &omangle))
    return;
  if (parse(line, length, "oarse", "ikelihood", "iff", FLOAT_TYPE, &obgdiff))
    return;
  if (parse(line, length, "oarse", "ikelihood", "raction", FLOAT_TYPE,
            &obgfrac))
    return;
  if (parse(line, length, "oarse", "tart", "adius", FLOAT_TYPE, &obrmin))
    return;
  if (parse(line, length, "oarse", "top", "adius", FLOAT_TYPE, &obrstop))
    return;
  if (parse(line, length, "ine", "pening", "ngle", FLOAT_TYPE, &cc0)) return;
  if (parse(line, length, "ine", "ngle", "ositive", FLOAT_TYPE, &cpangle))
    return;
  if (parse(line, length, "ine", "ngle", "egative", FLOAT_TYPE, &cmangle))
    return;
  if (parse(line, length, "ine", "ikelihood", "iff", FLOAT_TYPE, &cbgdiff))
    return;
  if (parse(line, length, "ine", "ikelihood", "raction", FLOAT_TYPE, &cbgfrac))
    return;
  if (parse(line, length, "ine", "inimum", "adius", FLOAT_TYPE, &cbrmin))
    return;
...
// *************************************************************
inline void fit_param::ang_constr(float &theta_0, float &theta_plus,
                                  float &theta_minus) {
...
void fit_param::print(void) {
  ...
  if (clusgtheta0 == FIT_PARAM_NONE) {
    ...
    if (clusgthetaplus > 0)
      printf(
          "Initial Clusfit angle negative deviation:  %8.2f deg\n",
          (acos(clusgtheta0) - acos(clusgtheta0 + sqrt(0.5 / clusgthetaplus))) /
              PI180);
    else
      printf("Initial Clusfit angle negative deviation:   none\n");
  }
  ...
  for (ar = 0; ar < np; ar++) {
    ...
    if (clusthetaplus[ar] > 0)
      printf("%8.2f deg\n",
             (acos(clustheta0[ar]) -
              acos(clustheta0[ar] + sqrt(0.5 / clusthetaplus[ar]))) /
                 PI180);
    else
      printf(" none\n");
